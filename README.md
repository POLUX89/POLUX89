...
|:--:|---|---|---|:--:|---|
| 🏋️ | **rehab-strength** | 🧪 **Nothing to upload** — one click loads a synthetic dataset, so a stranger can explore every tab with zero real health data · three data sources → weekly snapshot with freshness badges · SHAP-explained models · sleep-quality classifier **ROC-AUC 0.77 ± 0.15** (5-fold), wide enough that it is directional, not diagnostic | `Python` `Streamlit` `Pandas` | ![](https://img.shields.io/badge/live-2ea44f?style=flat) | **[Live app »](https://rehab-strength.streamlit.app)** · [Repo](https://github.com/POLUX89/rehab-strength) |
| 📰 | **NLP-Fake-News-Colombia** | BETO fine-tuned on **2,935 ColombiaCheck fact-checks** (harvested from ClaimReview markup) · macro-F1 **0.405** vs 0.386 TF-IDF baseline · `Verdadero` scores **0.00** and the model card says so — it is not a truth detector | `Transformers` `BETO` `Streamlit` | ![](https://img.shields.io/badge/live-2ea44f?style=flat) | **[Live demo »](https://beto-colombiacheck.streamlit.app/)** · [🤗 Model](https://huggingface.co/polux89/beto-colombiacheck) · [Repo](https://github.com/POLUX89/NLP-Fake-News-Colombia) |
| ⚡ | **Forecast-Energy-StockChange-Colombia** | **1.1M raw records → 101,256 gap-free hourly observations** of Colombia's spot price since 2015, refreshed daily by a self-updating GitHub Actions pipeline. Naive settlement-version ordering left **47% of hours stale** | `Python` `Time series` `GitHub Actions` | ![](https://img.shields.io/badge/in_progress-8a6100?style=flat) | [Repo](https://github.com/POLUX89/Forecast-Energy-StockChange-Colombia) |
